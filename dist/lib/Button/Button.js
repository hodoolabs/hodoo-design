...
const Button = ({ color, size, leftIcon, rightIcon, label, disabled, className, onClick }) => {
    const iconOnly = !label ? size : null;
    return (_jsxs("button", { type: 'button', className: `${cn(ButtonStyle({ color, size, iconOnly, disabled }))} ${className}`, disabled: disabled, onClick: onClick, children: [leftIcon && _jsx("div", { className: cn(ImageStyle({ size, iconOnly })), children: leftIcon }), label && _jsx("span", { children: label }), rightIcon && _jsx("div", { className: cn(ImageStyle({ size, iconOnly })), children: rightIcon })] }));
};
