...
const Input = forwardRef(({ type = 'text', size = 'lg', autoComplete = 'off', label, value, error, maxLength, placeholder, helper, disabled, required, className, onBlur, onChange, onError, onEnter, }, ref) => {
    useEffect(() => {
        if (!onError)
            return;
        onError('');
