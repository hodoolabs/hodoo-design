...
const Select = ({ size = 'lg', items, selected, error, center = false, label, placeholder, helper, disabled = false, required, className, onClick, onChange, onError, }) => {
    ...
    return (_jsxs(SelectStyled, { className: `flex flex-col ${className}`, onMouseLeave: () => setIsOpen(false), children: [_jsx(Label, { size: size, error: error, label: label, disabled: disabled, required: required }), _jsxs("div", { className: 'relative w-full', children: [_jsxs("button", { type: 'button', onClick: (event) => {
