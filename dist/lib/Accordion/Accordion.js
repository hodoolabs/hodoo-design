...
const Accordion = ({ list, path, className, onPush }) => {
    ...
    return (_jsx("div", { className: `text-base font-semibold flex flex-col gap-3 ${className}`, children: list.map((item) => (_jsxs("div", { children: [_jsxs("div", { className: cn(MenuStyle({ isCurrentPath: getIsCurrentPath(item.menu.path, path) })), onClick: () => handleMenuClick(item.index, expandedMenuIndex, item.menu.path, item.subMenus), children: [_jsx("img", { src: item[getIsCurrentPath(item.menu.path, path) ? 'activeIcon' : 'icon'], alt: item.icon, className: 'w-6 h-6' }), _jsx("span", { children: item.menu.label }), !!item.subMenus && (_jsx(ChevronDownIcon, { className: cn(ArrowStyle({ isExpanded: getIsExpandedMenu(item.index, expandedMenuIndex) })) }))] }), !!item.subMenus && getIsExpandedMenu(item.index, expandedMenuIndex) && (_jsx("div", { className: 'flex flex-col gap-1 overflow-hidden', children: item.subMenus.map((subItem, index) => (_jsx("div", { className: cn(SubMenuStyle({ isCurrentPath: getIsCurrentPath(subItem.path, path) })), onClick: () => onPush(subItem.path), children: subItem.label }, index))) }))] }, item.index))) }));
};
