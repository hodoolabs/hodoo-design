...
const Radio = ({ id, selected, label, helper, disabled = false, className, onChange }) => {
    return (_jsxs("div", { className: `inline-flex gap-3 ${className}`, children: [_jsx("button", { className: cn(RadioStyle({ selected: id === selected, disabled })), onClick: () => !disabled && onChange && onChange(id), children: id === selected && (disabled ? _jsx(RadioDisabledSvg, {}) : _jsx(RadioSvg, {})) }), label && (_jsxs("div", { className: 'space-y-1 font-medium', children: [_jsx("label", { className: cn(LabelStyle({ disabled })), onClick: () => !disabled && onChange && onChange(id), children: label }), helper && _jsx("p", { className: cn(HelperStyle({ disabled })), children: helper })] }))] }));
};
