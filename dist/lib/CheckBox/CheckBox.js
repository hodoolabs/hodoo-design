...
const CheckBox = ({ checked, color, label, helper, disabled = false, className, onChange }) => {
    return (_jsxs("div", { className: `inline-flex gap-3 ${className}`, children: [_jsx("button", { className: cn(CheckBoxStyle({ checked })), disabled: disabled, onClick: () => onChange(!checked), children: checked && (disabled ? _jsx(CheckedDisabledSvg, {}) : _jsx(CheckedSvg, { color: color })) }), label && (_jsxs("div", { className: 'flex flex-col gap-1', children: [_jsx("label", { className: cn(LabelStyle({ disabled })), onClick: () => !disabled && onChange(!checked), children: label }), helper && _jsx("p", { className: cn(HelperStyle({ disabled })), children: helper })] }))] }));
};
