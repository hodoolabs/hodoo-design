...
const Tbody = ({ size, columns, sortDatas }) => {
    return (_jsx("tbody", { children: sortDatas.map((record, index) => (_jsx("tr", { className: 'flex items-center font-medium border-b border-solid border-b-gray-200', children: columns.map((column, index) => (_jsx("td", { className: cn(TableBodyStyle({ size })), style: { width: `${column.width}%` }, children: column.render ? column.render(record) : record[column.dataIndex] }, index))) }, index))) }));
};
